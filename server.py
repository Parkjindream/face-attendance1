...
async def export_attendance(date: str = Query(None, description="YYYYMMDD")):
    """
    Export attendance ของวันที่ระบุ
    คืนเป็นไฟล์ zip ที่มี CSV + XLSX
    """
    # กำหนดวันที่ปัจจุบันถ้าไม่ส่ง
    if not date:
        date = _yyyymmdd_from_today()
    else:
        try:
            date = _normalize_date_token(date)
        except Exception:
            raise HTTPException(status_code=400, detail="รูปแบบวันที่ไม่ถูกต้อง")

    _ensure_export_files_for_date(date)

    files = []
    csv_path = f"attendance_{date}.csv"
    xlsx_path = f"attendance_{date}.xlsx"
    ...
    if not files:
        raise HTTPException(status_code=404, detail="ไม่พบไฟล์เช็คชื่อสำหรับวันนั้น")
    ...
    return StreamingResponse(mem_zip, media_type="application/zip", headers={
        "Content-Disposition": f"attachment; filename=attendance_{date}.zip"
    })
